...
inline void A20_resetWindDictDefault(ST_A20_WindDict_t& p_dict) {
    ...
    for (size_t v_i = 0; v_i < v_presetMax; v_i++) {
        A20_resetPresetEntryDefault(p_dict.presets[v_i]);
    }

    constexpr size_t v_styleMax = sizeof(p_dict.styles) / sizeof(p_dict.styles[0]);
    for (size_t v_i = 0; v_i < v_styleMax; v_i++) {
