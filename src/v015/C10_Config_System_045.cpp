    ...
    if (!j_ws.isNull()) {    
        ...
        if (!j_chArr.isNull()) {
            for (JsonObjectConst j_ch : j_chArr) {
                ...
                if (!j_ch["chIntervalMs"].isNull()) {
                    uint16_t v_new = C10_u16Clamp(j_ch["chIntervalMs"].as<uint32_t>(), 20, 60000);
                    p_cfg.system.webSocket.wsChConfig[v_idx].chIntervalMs = v_new;
                }

                // priority
                if (!j_ch["priority"].isNull()) {
                    uint8_t v_new = C10_u8Clamp(j_ch["priority"].as<uint32_t>(), 0, 10);
        ...
        if (!j_etc.isNull()) {
            if (!j_etc["chartLargeBytes"].isNull()) {
                uint16_t v_new = C10_u16Clamp(j_etc["chartLargeBytes"].as<uint32_t>(), 256, 60000);
                p_cfg.system.webSocket.wsEtcConfig.chartLargeBytes = v_new;
            }
            if (!j_etc["chartThrottleMul"].isNull()) {
                uint8_t v_new = C10_u8Clamp(j_etc["chartThrottleMul"].as<uint32_t>(), 1, 10);
                p_cfg.system.webSocket.wsEtcConfig.chartThrottleMul = v_new;
            }
            if (!j_etc["wsCleanupMs"].isNull()) {
                uint16_t v_new = C10_u16Clamp(j_etc["wsCleanupMs"].as<uint32_t>(), 200, 60000);
