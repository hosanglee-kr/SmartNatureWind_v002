...
// 		/**
// 		 * @param p_mutex 원본 SemaphoreHandle_t의 참조
// 		 * @param p_timeout 락 획득 대기 시간 (기본 100ms)
// 		 */
// 		CL_A40_MutexGuard_Semaphore(SemaphoreHandle_t& p_mutex, TickType_t p_timeout = pdMS_TO_TICKS(100))
// 			: _mutexPtr(&p_mutex) { // ✅ 참조의 주소를 포인터에 저장
...
// 		~CL_A40_MutexGuard_Semaphore() {
// 			if (_mutexPtr && *_mutexPtr && _acquired) {
// 				xSemaphoreGiveRecursive(*_mutexPtr);
// 			}
// 		}

// 		//  명시적 뮤텍스 획득 return true 획득 성공, false 실패
// 		bool acquire(uint32_t p_timeoutMs = portMAX_DELAY) {
// 			if (_acquired) return true; // 이미 획득 상태

// 			if (*_mutexPtr != nullptr) {
// 				if (xSemaphoreTakeRecursive(*m_mutexPtr, pdMS_TO_TICKS(p_timeoutMs)) == pdTRUE) {
// 					_acquired = true;
// 				}
// 			}
// 			return _acquired;
...
// 		//  명시적 뮤텍스 해제
// 		void unlock() {
// 			if (_acquired && *_mutexPtr != nullptr) {
// 				xSemaphoreGiveRecursive(*m_mutexPtr);
// 				_acquired = false;
// 			}
// 		}
...
// 		SemaphoreHandle_t* _mutexPtr; // 원본 핸들의 주소를 보관
// 		bool _acquired = false;
// };
...
// 		// SemaphoreHandle_t& 로 참조를 전달받아 원본 포인터를 직접 수정 가능하게 함
// 		CL_A40_MutexGuard_Semaphore(SemaphoreHandle_t& p_mutex, TickType_t p_timeout = pdMS_TO_TICKS(100))
// 			: _mutexPtr(p_mutex) {

// 			// 1. 뮤텍스가 없으면 여기서 자동 생성 (Thread-Safe하게 처리)
// 			if (*_mutexPtr == nullptr) {
// 				*_mutexPtr = xSemaphoreCreateRecursiveMutex();
// 			}
...
// 	private:
// 		SemaphoreHandle_t* _mutexPtr; // 원본 포인터 주소 저장
// 		bool _acquired = false;
// };
