...
/*
 * ------------------------------------------------------
 * 소스명 : A23_Com_ResetCfg_060.h   A20_Com_FuncRst_060.h
 * 모듈약어 : A20
...
// NOTE:
// - 본 헤더는 A20_Const_060.h에서 include하여 사용합니다.
// - A20_Const_060.h에 정의된 타입/상수/전역(예: A20_Const::*, ST_A20_*, EN_A20_*)에 의존합니다.
...
// [A20_resetPirDefault] PIR 공통 기본값 초기화
static inline void A20_resetPirDefault(ST_A20_PIR_t& p_pir) {
    p_pir.enabled = false;
    p_pir.holdSec = 0;
}

// [A20_resetBleDefault] BLE 공통 기본값 초기화
...
// [A20_resetMotionCommonDefault] Motion 공통 기본값(PIR/BLE) 초기화
...
// [A20_resetAutoOffDefault] AutoOff 기본값(timer/offTime/offTemp) 초기화
...
// [A20_resetSchAutoOffDefault] Schedule용 AutoOff 기본값 초기화(현재는 공통과 동일)
...
// [A20_resetAdjustDeltaDefault] AdjustDelta(가중치/델타) 기본값 초기화
...
// [A20_resetSchedulePeriodDefault] 스케줄 Period 기본값(매일, 00:00~23:59) 초기화
static inline void A20_resetSchedulePeriodDefault(ST_A20_SchedulePeriod_t& p_period) {
    for (uint8_t v_d = 0; v_d < 7; v_d++) {
        p_period.days[v_d] = 1;
    }
    A40_ComFunc::copyStr2Buffer_safe(p_period.startTime, "00:00", sizeof(p_period.startTime));
    A40_ComFunc::copyStr2Buffer_safe(p_period.endTime, "23:59", sizeof(p_period.endTime));
}
...
// [A20_resetScheduleSegmentDefault] 스케줄 Segment 기본값 초기화(프리셋/스타일 기본 코드 포함)
...
// [A20_resetResolvedWindDefault] ResolvedWind(시뮬 전달용) 기본값 초기화
...
// [A20_resetWindPresetFactorsDefault] Preset Factors 기본값 초기화
...
// [A20_resetPresetEntryDefault] PresetEntry 기본값 초기화(code/name empty + factors default)
...
// [A20_resetStyleFactorsDefault] Style Factors 기본값 초기화
...
// [A20_resetStyleEntryDefault] StyleEntry 기본값 초기화(code/name empty + factors default)
static inline void A20_resetStyleEntryDefault(ST_A20_StyleEntry_t& p_s) {
    memset(&p_s, 0, sizeof(p_s));
    A40_ComFunc::copyStr2Buffer_safe(p_s.code, "", sizeof(p_s.code));
    A40_ComFunc::copyStr2Buffer_safe(p_s.name, "", sizeof(p_s.name));
...
// [A20_resetPageAssetDefault] PageAsset 기본값 초기화
...
// [A20_resetPageItemDefault] PageItem 기본값 초기화(assets 포함)
...
// [A20_resetRedirectItemDefault] RedirectItem 기본값 초기화
...
// [A20_resetCommonAssetDefault] CommonAsset 기본값 초기화
...
// [A20_resetSystemDefault] System 설정 전체 기본값 초기화(meta/system/hw/security/timeCfg)
inline void A20_resetSystemDefault(ST_A20_SystemConfig_t& p_cfg) {
    memset(&p_cfg, 0, sizeof(p_cfg));

    // meta
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.meta.version,    A20_Const::FW_VERSION, sizeof(p_cfg.meta.version));
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.meta.deviceName, "SmartNatureWind",     sizeof(p_cfg.meta.deviceName));
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.meta.lastUpdate, "",                   sizeof(p_cfg.meta.lastUpdate));

    // system.logging
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.system.logging.level, "INFO", sizeof(p_cfg.system.logging.level));
    ...
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.hw.tempHum.type, "DHT22", sizeof(p_cfg.hw.tempHum.type));
...
// [A20_resetWifiDefault] WiFi 설정 기본값(AP+STA, AP SSID/PASS, STA list clear)
inline void A20_resetWifiDefault(ST_A20_WifiConfig_t& p_cfg) {
    memset(&p_cfg, 0, sizeof(p_cfg));

    p_cfg.wifiMode = EN_A20_WIFI_MODE_AP_STA;
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.wifiModeDesc, "0=AP,1=STA,2=AP+STA", sizeof(p_cfg.wifiModeDesc));

    // ap
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.ap.ssid, "NatureWind", sizeof(p_cfg.ap.ssid));
    A40_ComFunc::copyStr2Buffer_safe(p_cfg.ap.pass, "2540",       sizeof(p_cfg.ap.pass));
...
// [A20_resetMotionDefault] Motion 설정 기본값(pir/ble/rssi/timing) 초기화
...
// [A20_resetWindDictDefault] WindDict 기본값(엔트리 초기화 + 샘플 1개) 설정
inline void A20_resetWindDictDefault(ST_A20_WindDict_t& p_dict) {
    ...
    A40_ComFunc::copyStr2Buffer_safe(p_dict.styles[0].name, "Balance",  sizeof(p_dict.styles[0].name));
...
// [A20_resetScheduleItemDefault] ScheduleItem 기본값(period/segments/autoOff/motion) 초기화
...
// [A20_resetSchedulesDefault] SchedulesRoot 기본값(items 전체 초기화)
...
// [A20_resetUserProfileItemDefault] UserProfileItem 기본값(segments/autoOff/motion) 초기화
static inline void A20_resetUserProfileItemDefault(ST_A20_UserProfileItem_t& p_up) {
    memset(&p_up, 0, sizeof(p_up));

    p_up.profileId = 0;
    p_up.profileNo = 0;
    A40_ComFunc::copyStr2Buffer_safe(p_up.name, "", sizeof(p_up.name));
    p_up.enabled = true;
...
// [A20_resetUserProfilesDefault] UserProfilesRoot 기본값(items 전체 초기화)
...
// [A20_resetNvsSpecDefault] NVS Spec 기본값(namespace/entries 샘플) 초기화
inline void A20_resetNvsSpecDefault(ST_A20_NvsSpecConfig_t& p_cfg) {
    memset(&p_cfg, 0, sizeof(p_cfg));

    A40_ComFunc::copyStr2Buffer_safe(p_cfg.namespaceName, "SNW", sizeof(p_cfg.namespaceName));
    p_cfg.entryCount = 0;

    // 내부 유틸(함수 내에서만 사용)
    auto v_addEntry = [&](const char* p_key, const char* p_type, const char* p_def) {
        ...
        A40_ComFunc::copyStr2Buffer_safe(v_e.key,          p_key,  sizeof(v_e.key));
        A40_ComFunc::copyStr2Buffer_safe(v_e.type,         p_type, sizeof(v_e.type));
        A40_ComFunc::copyStr2Buffer_safe(v_e.defaultValue, p_def,  sizeof(v_e.defaultValue));
...
// [A20_resetWebPageDefault] WebPageConfig 기본값(pages/redirect/assets + 기본 메인/리다이렉트/공용자산) 초기화
inline void A20_resetWebPageDefault(ST_A20_WebPageConfig_t& p_cfg) {
    ...
    // redirect 추가(내부 유틸)
    auto v_addRedirect = [&](const char* p_from, const char* p_to) {
        if (p_cfg.reDirectCount >= A20_Const::MAX_REDIRECTS) return;
        ST_A20_ReDirectItem_t& v_r = p_cfg.reDirect[p_cfg.reDirectCount++];
        A40_ComFunc::copyStr2Buffer_safe(v_r.uriFrom, p_from, sizeof(v_r.uriFrom));
    ...
    auto v_addCommonAsset = [&](const char* p_uri, const char* p_path, bool p_isCommon) {
        ...
        A40_ComFunc::copyStr2Buffer_safe(v_c.uri,  p_uri,  sizeof(v_c.uri));
        A40_ComFunc::copyStr2Buffer_safe(v_c.path, p_path, sizeof(v_c.path));
...
// [A20_resetToDefault] ConfigRoot 포인터가 존재하는 항목만 기본값으로 초기화
