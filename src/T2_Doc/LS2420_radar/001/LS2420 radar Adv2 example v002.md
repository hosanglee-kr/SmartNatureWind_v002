...
private:
    ...
    SemaphoreHandle_t _mutex;          // Zone 배열 보호용 뮤텍스

    // Zone 배열
    ST_DetectionZone_t _zones[G_ZONE_COUNT];
...
bool CL_LD2420Radar::init(HardwareSerial& p_serial) {
    ...
    // 3. 센서 설정
    _radar.setDistanceRange(0, 400);
    _radar.setUpdateInterval(50);   // 50ms
...
void CL_LD2420Radar::_updateZones() {
    ...
    if (xSemaphoreTake(_mutex, portMAX_DELAY) == pdTRUE) {
        for (int v_i = 0; v_i < G_ZONE_COUNT; v_i++) {
...
void CL_LD2420Radar::_taskLoop() {
    // 실제로는 _radarTask에서 직접 호출하므로 빈 함수로 둠
    // (태스크 함수에서 직접 처리하므로 사용하지 않음)
}
...
이제 완성된 코드를 그대로 사용하거나, 필요에 따라 추가 기능을 확장하실 수 있습니다.
질문이나 수정 요청이 있으면 언제든지 말씀해주세요! 😊