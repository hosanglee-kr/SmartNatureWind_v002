...
#define GEMINI_TIMEOUT  15000  // 15초 (ESP32 메모리/CPU 부하 고려)
 
// 최소 호출 간격 (ms) – 무료 티어 레이트 리밋 방지
#define GEMINI_RATE_LIMIT_MS  3000
...
// Gemini API 호출 (POST)
// ------------------------------------------------------
static String callGeminiApi(const char* p_body, size_t p_len, int& r_httpCode) {
    const char* v_apiKey = getGeminiApiKey();
    ...
    v_client.setTimeout(GEMINI_TIMEOUT / 1000);
 
    CL_D10_Logger::log(EN_L10_LOG_DEBUG, "[W10][Gemini] Connecting to %s:%d...", GEMINI_HOST, GEMINI_PORT);
 
    if (!v_client.connect(GEMINI_HOST, GEMINI_PORT)) {
    ...
    while (v_client.connected() || v_client.available()) {
        if (millis() - v_startMs > (unsigned long)GEMINI_TIMEOUT) {
...
void CL_W10_WebAPI::routeGeminiProxy() {
    s_server->on(
        ...
        [](AsyncWebServerRequest* p_request) {
            p_request->send(400, "application/json", "{\"error\":\"empty body\"}");
        },
        // ─── 업로드 핸들러 없음 ───
        nullptr,
        // ─── 바디 핸들러 ───
        [](AsyncWebServerRequest* p_request, uint8_t* p_data, size_t p_len, size_t p_index, size_t p_total) {
            ...
            if (v_now - s_lastGeminiCallMs < GEMINI_RATE_LIMIT_MS) {
                p_request->send(429, "application/json",
                    "{\"error\":\"rate limited, retry after " +
                    String((GEMINI_RATE_LIMIT_MS - (v_now - s_lastGeminiCallMs)) / 1000) +
                    "s\"}");
                return;
            }
            ...
            // ─── Gemini 호출 ───
            int v_httpCode = 0;
            String v_resp = callGeminiApi((const char*)p_data, p_len, v_httpCode);
 
            CL_D10_Logger::log(
                EN_L10_LOG_INFO,
                "[W10][Gemini] Proxy call: HTTP %d, response %u bytes",
                v_httpCode, (unsigned)v_resp.length()
            );
