...
## LS2420 radar Adv2 example v007.md
...
// LED 및 모터 제어 설정 배열
constexpr ST_ZoneControl_t G_ZONE_CONTROLS[G_ZONE_ACTUAL_COUNT] = {
...
void vLEDMotorTask(void* pvParameters) {
    ...
    // 존 변경 시 캐시할 값
    int v_cachedMinDist = 0;
    int v_cachedMaxDist = 0;
    int v_cachedIdx = -1;   // 이전 존 인덱스 (변화 감지용)
    ...
    while (1) {
        ...
        if (v_state.distance < 0 || v_state.zone == E_ZONE_OUTOFRANGE) {
            ...
            v_ledOn = false;
            v_cachedIdx = -1;   // 존 정보 초기화
            vTaskDelayUntil(&v_lastWake, v_period);
            continue;
        }

        int v_idx = static_cast<int>(v_state.zone);
        if (v_idx < 0 || v_idx >= G_ZONE_ACTUAL_COUNT) {
            vTaskDelayUntil(&v_lastWake, v_period);
            continue;
        }

        // 존이 바뀌었으면 거리 범위를 다시 읽음 (한 번만)
        if (v_cachedIdx != v_idx) {
            v_cachedMinDist = v_radar.getZoneMinDist(v_idx);
            v_cachedMaxDist = v_radar.getZoneMaxDist(v_idx);
            v_cachedIdx = v_idx;
        }

        float v_t = fDistanceRatio(v_state.distance, v_cachedMinDist, v_cachedMaxDist);

        int v_blinkPeriod = G_ZONE_CONTROLS[v_idx].blinkFast_ms +
            static_cast<int>((G_ZONE_CONTROLS[v_idx].blinkSlow_ms - G_ZONE_CONTROLS[v_idx].blinkFast_ms) * v_t);

        int v_motorSpeed = G_ZONE_CONTROLS[v_idx].motorSpeedFast +
            static_cast<int>((G_ZONE_CONTROLS[v_idx].motorSpeedSlow - G_ZONE_CONTROLS[v_idx].motorSpeedFast) * v_t);
...
void vZoneChangeHandler(uint8_t p_zoneIndex, bool p_isActive, int p_distance) {
    if (xSemaphoreTake(g_stateMutex, portMAX_DELAY) == pdTRUE) {
        g_radarState.distance = p_distance;
