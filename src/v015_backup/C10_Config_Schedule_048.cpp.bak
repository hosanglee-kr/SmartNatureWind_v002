/*
 * ------------------------------------------------------
 * 소스명 : C10_Config_Schedule_048.cpp
 * 모듈 약어 : C10
...
static uint16_t C10_nextScheduleId(const ST_A20_SchedulesRoot_t& p_root) {
    ...
    if (v_next == 0 || v_next > 65535u) return (uint16_t)G_C10_SCH_ID_START;
    return (uint16_t)v_next;
}

static uint16_t C10_nextScheduleNo(const ST_A20_SchedulesRoot_t& p_root) {
    uint16_t v_max = 0;
    for (uint8_t v_i = 0; v_i < p_root.count && v_i < A20_Const::MAX_SCHEDULES; v_i++) {
        if (p_root.items[v_i].schNo > v_max) v_max = p_root.items[v_i].schNo;
    }
    if (v_max < (uint16_t)G_C10_SCH_NO_START) return (uint16_t)G_C10_SCH_NO_START;
    uint32_t v_next = (uint32_t)v_max + (uint32_t)G_C10_SCH_NO_STEP;
    if (v_next == 0 || v_next > 65535u) return (uint16_t)G_C10_SCH_NO_START;
    return (uint16_t)v_next;
}
...
bool CL_C10_ConfigManager::loadSchedules(ST_A20_SchedulesRoot_t& p_cfg) {
    ...
    if (arr.isNull()) {
        CL_D10_Logger::log(EN_L10_LOG_WARN, "[C10] %s: missing 'schedules' array (empty)", __func__);
        return true;
    }
    ...
    if (C10_isScheduleNoDuplicatedInList(p_cfg)) {
        CL_D10_Logger::log(EN_L10_LOG_ERROR, "[C10] %s: duplicated schNo detected. Load rejected.", __func__);
        A20_resetSchedulesDefault(p_cfg);
        return false;
    }
...
bool CL_C10_ConfigManager::loadUserProfiles(ST_A20_UserProfilesRoot_t& p_cfg) {
    ...
    if (arr.isNull()) {
        CL_D10_Logger::log(EN_L10_LOG_WARN, "[C10] %s: missing 'userProfiles.profiles' (empty)", __func__);
        return true;
    }
