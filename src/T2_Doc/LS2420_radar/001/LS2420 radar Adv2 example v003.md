...
- 객체지향 적용
- 하드코딩 및 매직넘버 금지
- 전역 상수는 constexpr 로 선언
- hpp, cpp, main.cpp로 구성
   main.cpp에서는
    include , 전역인스턴스, init, run함수만 호출
- 기능 축소/누락하지 말고 완성체 풀버전

- 추가 명명규칙
    - 로컬변수 v_ 접두사
    - 함수 파라메터 변수 p_접두사
    - 구조체 : ST_ 접두사 및  _t 접미사
    - 클래스명 : CL_ 접두사
    - private 맴버변수, private 맴버함수 : _ 접두사
    - 전역 상수 G_ 접두사
    - 전역 변수 g_ 접두사
    - 
...
private:
    ...
    // Zone 배열
    ST_DetectionZone_t _zones[EM_ZONE_t::E_ZONE_COUNT];
    
    static constexpr ST_DetectionZone_t DEFAULT_ZONES[E_ZONE_COUNT] = {
        [E_ZONE_CLOSE]  = {0, 50, "Close Range", false, 0},
        [E_ZONE_MIDDLE] = {51, 150, "Medium Range", false, 0},
        [E_ZONE_FAR]    = {151, 300, "Far Range", false, 0}
    };
    ...
    , _updateInterval_ms(50)
{
    ...
    memcpy(_zones, DEFAULT_ZONES, sizeof(_zones));
...
bool CL_LD2420Radar::init(HardwareSerial& p_serial) {
    ...
    /*
    // Zone 초기화
    _zones[EM_ZONE_t::E_ZONE_CLOSE] = {0, 50, "Close Range", false, 0};
    _zones[EM_ZONE_t::E_ZONE_MIDDLE] = {51, 150, "Medium Range", false, 0};
    _zones[EM_ZONE_t::E_ZONE_FAR] = {151, 300, "Far Range", false, 0};
...
void CL_LD2420Radar::_updateZones() {
    ...
    if (xSemaphoreTake(_mutex, portMAX_DELAY) == pdTRUE) {
        for (int v_i = 0; v_i < EM_ZONE_t::E_ZONE_COUNT; v_i++) {
...
bool CL_LD2420Radar::setZoneRange(uint8_t p_index, int p_minDist, int p_maxDist) {
    ...
    if (p_index >= EM_ZONE_t::E_ZONE_COUNT) {
...
bool CL_LD2420Radar::setUpdateInterval(unsigned long p_updateInterval_ms) {
    if (p_updateInterval_ms == 0) {
        _safePrintf("❌ Invalid UpdateInterval: (%d) \lu", p_updateInterval_ms);
        return false;
    }
    if (xSemaphoreTake(_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        xSemaphoreGive(_mutex);
        _safePrintf("✅ updateInterval set to %d ms\n", _updateInterval_ms);
        return true;
    }
    return false;
}
