...
## LS2420 radar Adv2 example v006.md
...
void CL_LD2420Radar::_updateZones() {
    ...
    if (callbackCopy) {
        for (int i = 0; i < changeCount; i++) {
            callbackCopy(changes[i].index, changes[i].active, v_currentDist);
        }
    }
}
...
void CL_LD2420Radar::_onStateChange(LD2420_DetectionState p_oldState,
    ...
    if (p_newState == LD2420_NO_DETECTION) {
        ...
        if (callbackCopy) {
            for (int i = 0; i < changeCount; i++) {
                callbackCopy(changes[i].index, false, -1);  // distance = -1
            }
        }
...
// 전역 상수 (하드웨어 설정, 변경 가능)
// ============================================================================
constexpr int G_LED_PIN        = 5;   // WS2812 데이터 핀
constexpr int G_NUM_LEDS       = 1;   // LED 개수
constexpr int G_MOTOR_PIN      = 18;  // 모터 PWM 핀
constexpr int G_LED_UPDATE_MS  = 10;  // LED 상태 갱신 주기 (ms)

// ============================================================================
// Zone별 제어 파라미터 (거리에 따라 선형 보간)
// ============================================================================
struct ST_ZoneControl_t {
    CRGB color;               // 기본 색상
    int  blinkFast_ms;        // 가장 가까울 때 점멸 주기(ms)
    int  blinkSlow_ms;        // 가장 멀 때 점멸 주기(ms)
    int  motorSpeedFast;      // 가장 가까울 때 모터 속도 (0~255)
    int  motorSpeedSlow;      // 가장 멀 때 모터 속도 (0~255)
};

constexpr ST_ZoneControl_t G_ZONE_CONTROLS[E_ZONE_ACTUAL_COUNT] = {
...
// 전역 변수 (g_ 접두사)
// ============================================================================
CRGB g_leds[G_NUM_LEDS];                     // LED 배열
TaskHandle_t g_taskHandleLED = nullptr;      // LED/모터 제어 태스크 핸들
SemaphoreHandle_t g_stateMutex = nullptr;    // 상태 변수 보호 뮤텍스

// 콜백과 태스크 간 공유 상태
struct ST_RadarState_t {
    int       distance;       // 현재 거리 (cm), -1이면 미감지
    EM_ZONE_t zone;           // 현재 존
    bool      zoneChanged;    // 존 변경 플래그 (태스크에서 초기화)
};
ST_RadarState_t g_radarState = { -1, E_ZONE_OUTOFRANGE, false };

// ============================================================================
// 유틸리티 함수: 거리 비율 (0.0: 가까움, 1.0: 멀음)
// ============================================================================
float fDistanceRatio(int p_distance, int p_minDist, int p_maxDist) {
    ...
    return (float)(p_distance - p_minDist) / (p_maxDist - p_minDist);
}
...
void vLEDMotorTask(void* pvParameters) {
    ...
    // Zone 거리 범위 (setup과 동기화)
    constexpr int G_ZONE_MIN[E_ZONE_ACTUAL_COUNT] = {0, 51, 151, 301};
    constexpr int G_ZONE_MAX[E_ZONE_ACTUAL_COUNT] = {50, 150, 300, 500};
    ...
    while (1) {
        ...
        if (xSemaphoreTake(g_stateMutex, portMAX_DELAY) == pdTRUE) {
            ...
            g_radarState.zoneChanged = false;   // 플래그 소비
            xSemaphoreGive(g_stateMutex);
        }

        // 1) 미감지 또는 Out of Range → 전부 OFF
        if (v_state.distance < 0 || v_state.zone == E_ZONE_OUTOFRANGE) {
            FastLED.clear();
            FastLED.show();
        ...
        // 2) 존 인덱스 유효성 검사
        int v_idx = static_cast<int>(v_state.zone);
        if (v_idx < 0 || v_idx >= E_ZONE_ACTUAL_COUNT) {
            vTaskDelayUntil(&v_lastWake, v_period);
            continue;
        }

        // 3) 거리 비율
        float v_t = fDistanceRatio(v_state.distance, G_ZONE_MIN[v_idx], G_ZONE_MAX[v_idx]);

        // 4) 점멸 주기
        int v_blinkPeriod = G_ZONE_CONTROLS[v_idx].blinkFast_ms +
            (int)((G_ZONE_CONTROLS[v_idx].blinkSlow_ms - G_ZONE_CONTROLS[v_idx].blinkFast_ms) * v_t);

        // 5) 모터 속도
        int v_motorSpeed = G_ZONE_CONTROLS[v_idx].motorSpeedFast +
            (int)((G_ZONE_CONTROLS[v_idx].motorSpeedSlow - G_ZONE_CONTROLS[v_idx].motorSpeedFast) * v_t);
        analogWrite(G_MOTOR_PIN, v_motorSpeed);

        // 6) LED 점멸 처리
        unsigned long v_now = millis();
        if (v_now - v_lastBlinkToggle >= v_blinkPeriod / 2) {
            v_ledOn = !v_ledOn;
            v_lastBlinkToggle = v_now;
        }

        if (v_ledOn) {
            g_leds[0] = G_ZONE_CONTROLS[v_idx].color;
        } else {
            g_leds[0] = CRGB::Black;
        }
        FastLED.show();
...
void setup() {
    ...
    Serial.println("=== LD2420 + FreeRTOS + FastLED + Motor (Event Driven) ===");
    ...
    v_radar.onZoneChange(vZoneChangeHandler);   // 콜백 등록
