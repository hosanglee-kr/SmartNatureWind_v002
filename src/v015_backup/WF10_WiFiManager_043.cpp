...
bool CL_WF10_WiFiManager::startSTA(const ST_A20_WifiConfig_t& p_cfg_wifi, WiFiMulti& p_multi, uint8_t p_maxTries) {
    // 1. 가드 클래스 적용 (함수 시작부터 끝까지 보호)
    CL_A40_MutexGuard_Semaphore v_guard(s_wifiMutex, G_TM10_MUTEX_TIMEOUT);
    if (!v_guard.isAcquired()) {
        // [WF10]으로 모듈 약어 통일 및 __func__ 사용
    ...
    for (uint8_t v_i = 0; v_i < p_cfg_wifi.staCount; v_i++) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_INFO, "[WF10] %s: STA candidate: %s", __func__, v_ssid);
    }

    // 2. 가드 잠시 해제 (중요!)
    // WiFiMulti::run()이나 delay()는 시간이 오래 걸리는 Blocking 작업입니다.
    // 이 동안 뮤텍스를 잡고 있으면 다른 태스크(예: toJson)가 응답하지 못합니다.
    // 상태 설정을 마쳤으므로 run 루프 진입 전 가드를 해제하는 것이 운영상 유리합니다.
    v_guard.unlock();
