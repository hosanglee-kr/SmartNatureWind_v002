...
/*
 * ------------------------------------------------------
 * 소스명 : A22_Com_Lookup_060.h  A20_Com_Func_060.h
 * 모듈약어 : A20
...
// [A20_modeFromString] segment mode code(string) -> enum 변환
...
// [A20_modeToString] enum -> segment mode code(string) 변환
...
// ======================================================
// 6) WindDict 검색 유틸 (Static Dict / Runtime Dict)
// ======================================================

// [A20_getStaticPresetIndexByCode] 정적 프리셋(code) -> 정적 인덱스 반환(없으면 -1)
...
// [A20_getPresetNameByCode] 프리셋(code) -> 표시용 name 반환(없으면 "Unknown")
inline const char* A20_getPresetNameByCode(const char* p_code) {
    int8_t v_idx = A20_getStaticPresetIndexByCode(p_code);
    if (v_idx >= 0) return G_A20_WindPreset_Arr[(uint8_t)v_idx].name;
    return "Unknown";
}

// [A20_findPresetIndexByCode] 런타임 dict에서 preset(code) 인덱스 검색(없으면 -1)
...
// [A20_findStyleIndexByCode] 런타임 dict에서 style(code) 인덱스 검색(없으면 -1)
...
// ======================================================
// Lookup Utils (운영급 공용 유틸) - Static Table 기반
// ======================================================

// [A20_findPresetByCode] 정적 프리셋 테이블에서 preset(code) 포인터 반환(없으면 nullptr)
inline const ST_A20_WindPreset_t* A20_findPresetByCode(const char* p_code) {
    if (!p_code || !p_code[0]) return nullptr;

    for (uint8_t v_i = 0; v_i < EN_A20_WINDPRESET_COUNT; v_i++) {
        const char* v_code = G_A20_WindPreset_Arr[v_i].code;
        if (v_code && (strcasecmp(p_code, v_code) == 0)) return &G_A20_WindPreset_Arr[v_i];
    }
    return nullptr;
}

// [A20_findStyleByCode] 정적 스타일 테이블에서 style(code) 포인터 반환(없으면 nullptr)
inline const ST_A20_WindStyles_t* A20_findStyleByCode(const char* p_code) {
    if (!p_code || !p_code[0]) return nullptr;

    for (uint8_t v_i = 0; v_i < EN_A20_WINDSTYLE_COUNT; v_i++) {
        const char* v_code = G_A20_WindStyle_Arr[v_i].code;
        if (v_code && (strcasecmp(p_code, v_code) == 0)) return &G_A20_WindStyle_Arr[v_i];
    }
    return nullptr;
}

// [A20_findSegModeByCode] 정적 세그먼트 모드 테이블에서 segMode(code) 포인터 반환(없으면 nullptr)
inline const ST_A20_SegmentMode_t* A20_findSegModeByCode(const char* p_code) {
    if (!p_code || !p_code[0]) return nullptr;

    for (uint8_t v_i = 0; v_i < EN_A20_SEG_MODE_COUNT; v_i++) {
        const char* v_code = G_A20_SegmentMode_Arr[v_i].code;
        if (v_code && (strcasecmp(p_code, v_code) == 0)) return &G_A20_SegmentMode_Arr[v_i];
    }
    return nullptr;
}
