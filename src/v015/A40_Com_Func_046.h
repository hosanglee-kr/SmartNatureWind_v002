...
/*
 * ------------------------------------------------------
 * 소스명 : A20_IO_Util_001.h
 * 모듈약어 : A20
 * 모듈명 : Smart Nature Wind 공용 IO 유틸 (LittleFS + JSON 트랜잭션) (v001)
 * ------------------------------------------------------
 * 기능 요약:
 *  - LittleFS JSON 로드/세이브 운영급 유틸
 *  - 로드: main 우선 파싱 → 실패/부재 시 .bak 파싱/복구(백업 유지 정책)
 *  - 세이브: .tmp에 먼저 쓰기 → (선택) .bak 백업 → 원자적 교체(rollback 지원)
 *  - 선택: pretty/compact 저장, readback 검증(옵션)
 * ------------------------------------------------------
 * [구현 규칙]
